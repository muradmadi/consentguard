...
2. Run the unified test suite:
   ...
   ```
   *Ensure all server unit tests, engine pipeline tests, and client adapter tests pass.*
...
2. Publish the monorepo workspaces:
   ...
   ```
   *Note: Ensure you are publishing the `dist` directories and not raw TypeScript files by verifying the `files` array in each `package.json`.*
