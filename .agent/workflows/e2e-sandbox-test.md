---
description: Running local end-to-end sandbox tests across client, proxy, and Redis.
---
...
3. Start the ConsentGuard Proxy server in development mode:
   ...
   ```
   _The proxy should start on `http://localhost:3000`._
...
1. Send a `PUT` request to the proxy to establish the consent state for a test user (`test_user_123`).
   ...
   ```
   _Note: In this scenario, `analytics` is denied, but `marketing` is allowed._
...
1. Open the local sandbox HTML file provided in the repository (e.g., `packages/client/sandbox/index.html`) in your browser.
   _(You may need to serve it via a simple HTTP server like `npx serve packages/client/sandbox`)._
