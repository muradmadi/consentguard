...
describe('scrubUrl value scan', () => {
  ...
  it('strips a raw IP address out of the query', () => {
    const result = scrubUrl('https://api.vendor.test/track?ip=203.0.113.9&e=view', BARE, { hasher })
    const params = new URL(result.url).searchParams
  ...
  it('scrubs every value of a repeated parameter', () => {
    ...
    expect(result.report).toEqual([
      { path: '?u.0', action: 'hash', mode: 'pseudonymize', matched: 1, detector: 'email' },
      { path: '?u.2', action: 'hash', mode: 'pseudonymize', matched: 1, detector: 'email' },
    ])
  })

  it('never records the value it removed', () => {
    const result = scrubUrl('https://api.vendor.test/track?em=alice@example.com', BARE, { hasher })
...
describe('scrubUrl declared rules', () => {
  it('applies a declared transformation to a matching query parameter', () => {
    ...
    expect(result.report).toEqual([
      { path: '?uid', action: 'hash', mode: 'pseudonymize', matched: 1 },
...
describe('scrubUrl leaves alone what it must', () => {
  it('returns the url byte-identical when nothing fires', () => {
    ...
    expect(scrubUrl(url, BARE, { hasher })).toEqual({ url, report: [] })
  })
  ...
  it('changes nothing when the scan is switched off and no rule declares a path', () => {
    ...
    expect(scrubUrl(url, BARE, { detectors: [], hasher })).toEqual({ url, report: [] })
  })
})
