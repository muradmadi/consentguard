...
function makeContext(overrides: Partial<VendorContext> & { env?: Record<string, string> } = {}): VendorContext {
  ...
  return {
    method: 'POST',
    originalUrl: overrides.originalUrl ?? 'https://www.google-analytics.com/g/collect?v=2&tid=G-TEST12345&cid=abc.def&en=page_view&ep.page_title=Home&epn.value=42',
...
describe('GA4 adapter', () => {
  ...
  it('hashes an email that appears in event params (rule-driven scrubbing)', async () => {
    const ctx = makeContext({
      originalUrl: 'https://www.google-analytics.com/g/collect?v=2&tid=G-TEST12345&cid=abc&en=signup&ep.email=alice@example.com',
    });
    const result = await ga4Adapter.buildRequest(ctx);
    if (!result || 'skip' in result) throw new Error('expected forward');
    const body = JSON.parse(result.body);
