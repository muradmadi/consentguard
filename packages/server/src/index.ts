...
const storage = serverConfig.env === 'test' || env.SLUICE_STORAGE === 'memory'
...
if (serverConfig.allowedOrigins.length === 0) {
  console.warn('[Sluice] SLUICE_ALLOWED_ORIGINS is not set — the proxy will accept any origin. Set it in production.')
}
if (!serverConfig.ga4.measurementId || !serverConfig.ga4.apiSecret) {
  console.warn('[Sluice] GA4_MEASUREMENT_ID / GA4_API_SECRET not set — GA4 forwarding is disabled (events will 204).')
}
